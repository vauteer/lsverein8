...
- Club scoping is via `#[ScopedBy]` attribute classes, not `booted()` closures. `ClubScope` (Item, Member, Subscription, Section) restricts to `currentClubId()`; `ClubWithSharedScope` (Event, Role) also lets `club_id IS NULL` rows through. Which one a model gets follows its column: only `events` and `roles` still have `club_id` nullable. `items` and `subscriptions` are NOT NULL, so they take the plain scope — Item carried `ClubWithSharedScope` until 2026-08-26, where the `IS NULL` disjunct simply never matched, and Section until 2026-08-30, when `sections.club_id` became NOT NULL. Both wrap the condition in a nested closure — lsverein7 did not, so its `orWhere` leaked past other conditions and returned other clubs' rows.
