...
Fallstrick beim Testen: fünf Auswahlen (`milestone_birthdays`, `deaths`, `joined`, `left`, `due_honours`) hängen an MySQL-only-Scopes, die SQLite nicht ausführt. `MemberManagementTest` prüft sie über die `QueryException` des Prepared Statements — nicht versuchen, sie über HTTP grün zu bekommen.
