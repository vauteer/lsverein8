...
return [
    ...
    */
    ...
    |
    | `auth` is added in front of the package's own list: Authorize resolves
    | the viewTelescope gate, which does deny a guest, but only with a bare
    | 403. With `auth` a signed-out visitor gets the login screen instead —
    | same arrangement as config/log-viewer.php.
    |
    */
    ...
    */
    ...
    /*
     * `log-viewer*` is ours: its UI polls its own API while it is open, which
     * would otherwise bury the requests one actually came to look at.
     */
    'ignore_paths' => [
        'livewire*',
        'nova-api*',
